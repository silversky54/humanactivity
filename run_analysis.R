...
selectactivityData<-select(activityData, recordN,subject, activity_id, tgroup, matches("[Mm]ean[(]|[Ss]td[(]")) %>%
        ...
        unite(feature, feature, axis, sep="")%>%
...
selectactivityData<-mutate(selectactivityData, feature=sub("^t","Time",feature))%>%
        mutate(feature=sub("^f","Freq",feature)) %>%
        rename(record_number=recordN, group=tgroup, activity_name=activity_nm)
...
averageactivitydata<-select(selectactivityData, subject, activity_name, feature, mean, std) %>%
        group_by(activity_name, subject, feature) %>%
